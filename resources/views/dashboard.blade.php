...
@section('content')
    ...
    <script>
        ...
        // ── STATE POLLING ──────────────────────────────────────────────────────────────
        // Menyimpan ISO timestamp dari polling sebelumnya — digunakan untuk mendeteksi
        // apakah ada data baru DARI CACHE (live), bukan dari DB.
        ...
        function perbaruiDashboard() {
            fetch('/live-data')
                ...
                .then(function(data) {
                    ...
                    if (!isOnline) {
                        // ── OFFLINE: kosongkan semua card sensor ─────────────────
                        setCardOffline();
                    ...
                    var tsTime      = data.timestamp_time;

                    // Update header timestamp selalu (menunjukkan polling aktif)
                    var elAt = document.getElementById('live-created-at');
                    if (elAt && tsFmt) elAt.innerText = tsFmt;

                    // Hanya update card jika ada data baru dari IoT (timestamp berubah)
                    if (tsIso === lastLiveTimestamp) return;
                    lastLiveTimestamp = tsIso;

                    // 1. Update angka card sensor
                    el('live-suhu',  suhu);
                    el('live-udara', udara);
                    el('live-tanah', tanah);

                    // 2. Update sub-teks kondisi
                    ...
                    // 4. Update meter pointer & panel analisis fuzzy
                    ...
                    // 6. Grafik hanya diperbarui jika ada data baru dari DB (via manual atau 15 menit)
                    // (Chart.js di-update oleh tombol manual saja; polling grafik cukup dari load awal)
                })
                .catch(function(err) {
        ...
        // ── FUNGSI OFFLINE: kosongkan semua elemen sensor ─────────────────────────────
        ...
        perbaruiDashboard(); // langsung panggil sekali saat halaman dibuka
